...
int ftp_nlist (int sck, int verbose) {
	...
	buffer = (char *) malloc(1024 * sizeof(char));
		...
		if (buffer == NULL) {
		printf("Unable to alloc 1024 bytes to buffer (ftp_nlist)\n");
		return -1;
		}
    ...
    if ( (con_err = connect(sock, (struct sockaddr *)&ftp_connection, sizeof(struct sockaddr)) ) != 0)  {
		...
		data_port = 0;
		n = recv(sck, buffer, 1023, 0);
		memset(buffer, 0x0, 1024);
		return DATA_CONNECTION_ERROR;
    }

	n = 0;
	nlist = fopen("NLIST.txt", "w");
	if (nlist == NULL) {
		printf("Unable to open NLIST file..\n");
		free(buffer);
