 ...
 */
  ...
  fprintf (p->logfd, "%16lf int0/msec\t%16lf msec/int0\n",
	   (double) p->m_int0 * p->m_freq / ((p->m_crtm - p->m_sttm)) / 1000,
	   (double) (p->m_crtm - p->m_sttm) / (p->m_int0 * p->m_freq) * 1000);
  fprintf (p->logfd, "%16lf int1/msec\t%16lf msec/int1\n",
	   (double) p->m_int1 * p->m_freq / ((p->m_crtm - p->m_sttm)) / 1000,
	   (double) (p->m_crtm - p->m_sttm) / (p->m_int1 * p->m_freq) * 1000);
  fprintf (p->logfd, "%16lf int2/msec\t%16lf msec/int2\n",
	   (double) p->m_int2 * p->m_freq / ((p->m_crtm - p->m_sttm)) / 1000,
	   (double) (p->m_crtm - p->m_sttm) / (p->m_int2 * p->m_freq) * 1000);
/*
 */
...
pic16f648OpCall (pic16f648_t * p, uint16_t op)
{
  ...
  PUSH (p, p->pc + 1);
  SETPC (p, ((p->reg.bank0[PCLATH] & 0x0018) << 8) | (op & 0x07ff));
  addWait (p);
  return 0;
}
...
//fprintf(p->logfd, "0070: %02x %02x %02x %02x %02x %02x %02x %02x\n", p->reg.m[0x0070], p->reg.m[0x0071], p->reg.m[0x0072], p->reg.m[0x0073], p->reg.m[0x0074], p->reg.m[0x0075], p->reg.m[0x0076], p->reg.m[0x0077]);
	  p->reg.bank0[INTCON] = p->reg.bank1[INTCON]
	    = p->reg.bank2[INTCON] = p->reg.bank3[INTCON] |= T0IF;
#ifdef MEASUREMENT
	  p->m_int0++;
#endif /* MEASUREMENT */
	  if (p->intr == 0)
...
//                              tid = 1 << p->rcvbuf[1];
	      tid = p->rcvbuf[1] + 1;
	      log =
		((uint32_t) p->rcvbuf[4] << 16) | ((uint32_t) p->
						   rcvbuf[5] << 8) |
		((uint32_t) p->rcvbuf[6] << 0);
	      for (i = 0; i < 24; i++)
		{
		  rid = 1 << i;
		  if ((log & rid) != 0)
 ...
 */
  ...
  if ((p->reg.m[OPTREG] & PSA) == 0)
    {				/* prescaler in use */
      if (p->presc0 == (1 << ((p->reg.m[OPTREG] & 0x07) + 1)))
	{
	  p->presc0 = 0;
	  if ((p->reg.bank0[TMR0] = p->reg.bank2[TMR0]++) == 0xff)
	    {
	      p->reg.bank0[INTCON]
		= p->reg.bank1[INTCON]
		= p->reg.bank2[INTCON] = p->reg.bank3[INTCON] |= T0IF;
//fprintf(p->logfd, "timer0 prescaled interrupt at %lf\n", (double)p->m_step / 1000000);
	      if ((p->reg.bank0[INTCON] & GIE) && (p->reg.m[INTCON] & T0IE))
		{
#ifdef MEASUREMENT
		  p->m_int0++;
#endif /* MEASUREMENT */
    ...
    {				/* prescaler not in use */
      if ((p->reg.bank0[TMR0] = p->reg.bank2[TMR0]++) == 0xff)
	{
	  p->reg.bank0[INTCON] = p->reg.bank1[INTCON]
	    = p->reg.bank2[INTCON] = p->reg.bank3[INTCON] |= T0IF;
//fprintf(p->logfd, "timer0 interrupt at %lf\n", (double)p->m_step / 1000000);
	  if ((p->reg.m[INTCON] & GIE) && (p->reg.m[INTCON] & T0IE))
	    {
#ifdef MEASUREMENT
	      p->m_int0++;
#endif /* MEASUREMENT */
...
//fprintf(p->logfd, "timer1 interrupt at %lf\n", (double)p->m_step / 1000000);
	      if ((p->reg.m[INTCON] & GIE) && (p->reg.m[PIE1] & TMR1IE))
		{
#ifdef MEASUREMENT
		  p->m_int1++;
#endif /* MEASUREMENT */
...
#endif /* INTLOG */
	  p->reg.m[TMR2] = 0;
	  if (p->pstsc2++ == ((p->reg.m[T2CON] & 0x78) >> 3))
	    {
#ifdef INTLOG
...
//fprintf(p->logfd, "timer2 interrupt at %lf\n", (double)p->m_step / 1000000);
	      if ((p->reg.m[INTCON] & GIE) && (p->reg.m[PIE1] & TMR2IE))
		{
#ifdef INTLOG
  ...
  else if (p->presc2 == (1 << ((p->reg.m[T2CON] & T2CKPS0) * 2)) - 1)
    {
#ifdef INTLOG
      fprintf (p->logfd, "%16llu: timer2 prescaler overflow.\n", p->m_step);
#endif /* INTLOG */
...
//if(p->pc == 0x0481){fprintf(p->logfd, "recieved packet has slot number %d\n", p->reg.m[0x0044]);}
      ...
      if (p->pc == 0x03be)
	{
	  p->reg.m[TXSTA] |= 0x02;	/*fprintf(p->logfd, "TXSTA = %02x\n", p->reg.m[0x0098]); */
	}
...
//if(p->pc == 0x01b5){p->reg.m[0x0061] = (int)((float)((random() & 0xffff0000) >> 16) / 65536 * 9) + 2;}
      if (p->pc == 0x01b5)
	{
	  p->reg.m[0x0061] = ((random () & 0xffff0000) >> 16) % 9 + 2;
	}
