...
tag_sender (void *p)
{
  ...
  for (;;)
	...
	goto skip;
      ...
      if (s->cpu->rcvstat == PENDING)
	{
	  fprintf (s->cpu->logfd, "%2d: packet collide with sending packet\n",
		   s->cpu->gsid);
	  fflush (s->cpu->logfd);
	}
      s->cpu->rcvscnt = 15808;
...
//      fprintf(s->cpu->logfd, "log started\n"); fflush(s->cpu->logfd);
  if (s->running == 0)
	...
	{
	  fprintf (stderr, "failed to start processor instance\n");
	  return -1;
	}
    }
  for (;;)
...
//      fprintf(s->cpu->logfd, "r -t %f -Ni %d -Nl MAC -Ms %d -Md %d -It AODV\n", (double)s->cpu->m_step / 1000000, s->gsid, ntohl(packet->sgsid), ntohl(packet->dgsid)); fflush(s->cpu->logfd);
  while (s->running == 0)
    goto out;
  if (ntohl (packet->data.len) != 8)
    fprintf (stderr, "packet size (%d) incorrect\n",
